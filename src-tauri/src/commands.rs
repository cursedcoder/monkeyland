...
use crate::wm_brain::{WmEvent, WmState};
use crate::wm_commands_inner;
...
pub async fn wm_launch(
    ...
    let result =
        wm_commands_inner::wm_launch_inner(&env, &meta_db, &orch_state, &registry, &prompt_text)?;
...
pub async fn wm_handle_message(
    ...
    let result =
        wm_commands_inner::wm_handle_message_inner(&env, &meta_db, &orch_state, &registry, &text)?;
    for event in &result.events {
        emit_wm_event(&app, event)?;
    }
    Ok(())
...
pub async fn wm_llm_done(
    ...
    let result = wm_commands_inner::wm_llm_done_inner(&meta_db, &orch_state, &response_text)?;
    for event in &result.events {
        emit_wm_event(&app, event)?;
    }
    Ok(())
...
pub async fn wm_llm_error(
    ...
    let result = wm_commands_inner::wm_llm_error_inner(&meta_db, &error)?;
    for event in &result.events {
        emit_wm_event(&app, event)?;
    }
    Ok(())
...
pub async fn wm_get_state(meta_db: State<'_, MetaDb>) -> Result<WmState, String> {
    Ok(wm_commands_inner::load_wm_state(&meta_db))
}
